...
func createEmail(client pb.MailingListServiceClient, addr string) *pb.EmailEntry {
	...
	// if the function completes in under a second, cancel to free resources for remainder of second
	defer cancel()
...
func main() {
	...
	// .Dial() connects to grpc srver with address and credentials
	// we're not requiring encryption, authentication, login, etc.
	...
	// this associates the client with the rpc messages we defined
	client := pb.NewMailingListServiceClient(conn)

	// newEmail := createEmail(client, "995@999.999")
